...
test("story mode map tuning starts with zombie waves and without boss encounters", async ({ page }) => {
  ...
  await expect
    ...
    .toBe(false);
  await expect
    .poll(() => page.evaluate(() => (window.__prototypeDebug as any)?.storyLegacyOverlayCount ?? -1))
    .toBe(0);
  await expect
  ...
  await expect
    ...
    .toBeUndefined();
  await expect
    .poll(() => page.evaluate(() => window.__prototypeDebug?.story2_5dProjectedDistrictUnderlayAlpha))
    .toBeUndefined();
  await expect
    .poll(() =>
      page.evaluate(() => {
